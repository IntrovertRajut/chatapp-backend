...
const getMyChats = TryCatch(async (req, res, next) => {
  ...
  const transformedChats = chats.map(({ _id, name, members, groupChat }) => {
    ...
    return {
      ...
      groupChat,
      avatar: groupChat
        ? members.slice(0, 3).map(({ avatar }) => avatar?.url || 'default-avatar.png') 
...
const getMyGroups = TryCatch(async (req, res, next) => {
  ...
  const groups = chats.map(({ members, _id, groupChat, name }) => ({
    ...
    groupChat,
    name,
    avatar: members.slice(0, 3).map(({ avatar }) => avatar?.url || 'default-avatar.png'), // Ensure avatar is checked
  }));
...
const getChatDetails = TryCatch(async (req, res, next) => {
  if (req.query.populate === "true") {
    ...
    chat.members = chat.members.map(({ _id, name, avatar }) => ({
      _id,
      name,
      avatar: avatar?.url || 'default-avatar.png', // Ensure avatar is checked
    }));
